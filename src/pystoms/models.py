...
class AbstractModel(pm.Model):
    ...
    def evaluation(self,prior_pred:bool=False,post_pred:bool=False,post_sample_n:int=100,save_fig:bool=False,path_output_dir:str="Evaluation"):
        """Evaluation of model
        ...
        if self.syn == None:
            raise ValueError("This model instance did not receive synthetic data or no SyntheticPeptideFeature instance was passed.")
        ...
        # trace plot
        az.plot_trace(self.trace,chain_prop={"color":["green","red","blue","black"]})
        if save_fig:
        ...
        Fig, axs = plt.subplots(nrows=plot_length,ncols=2,figsize=(plot_length*4,10))
    ...
    def _summary(self,ref_values:dict):
        ...
        summary["deviation_(%)"] = (summary["reference"]-summary["mean"])/summary["mean"]*100
    ...
    def priorAnalysis(self,save_fig:bool=False,path_output_dir:str="Evaluation"):
    ...
    def posteriorAnalysis(self,post_sample_n:int=100,save_fig:bool=False,path_output_dir:str="Evaluation"):
        ...
        rand_ind = np.random.choice(self.trace.posterior["draw"],size=post_sample_n,replace=False)
        subset_trace = self.trace.sel(draw=rand_ind)
        subset_trace.posterior["draw"] = np.arange(post_sample_n)

        with self as model:
            self.posterior_predictive = pm.sample_posterior_predictive(subset_trace,keep_size=True)
...
class ModelEMG(AbstractModel):
    ...
    def __init__(self,rt_values:np.ndarray,μ_hat:float=None,σ_hat:float=None,λ_hat:float=1.,name:str="",model:pm.Model=None,syn:SyntheticPeptideFeature=None):
        ...
        # Likelihood
        self.Var("obs",pm.ExGaussian.dist(mu=self.emg_μ,sigma=self.emg_σ,nu=self.emg_λ),data=rt_values)
...
class ModelIsotopicPattern(AbstractModel):
    ...
    """
    def __init__(self,mz_values:np.ndarray,z:int=1,n:int=6,μ_mass:float=None,σ_mass:float=1,σ_iso:float=0.1,name:str="",model:pm.Model=None,syn:SyntheticPeptideFeature=None):
        ...
        weights = pmath.exp(-lambda_averagine)*lambda_averagine**num_iso/factorial(num_iso)
        weights_n = weights/pmath.sum(weights)
        self.Var("sigma",pm.Exponential.dist(lam=1/σ_iso))

        # Likelihood
        self.Var("obs",pm.NormalMixture.dist(w=weights_n,mu=masses,sigma=self.sigma),data=obs_masses)
...
class ModelChargeIsotopicPattern(AbstractModel):
    ...
    """
    def __init__(self,mz_values:np.ndarray,z_start:int=2,n:int=6,μ_mass:float=None,σ_mass:float=1,σ_iso:float=0.1,name:str="",model:pm.Model=None,syn:SyntheticPeptideFeature=None):
        """Initialize isotopic pattern model
        ...
        # Likelihood
        self.Var("obs",pm.NormalMixture.dist(w=weights_n,mu=mzs,sigma=self.sigma),data=mz_values)
...
class ModelChargeAveragineIsotopicPattern(AbstractModel):
    ...
    """
    def __init__(self,mz_values:np.ndarray,z_start:int=2,n:int=6,μ_mass:float=None,σ_mass:float=1,σ_iso:float=0.1,λ_a_hat=0.01,name:str="",model:pm.Model=None,syn:SyntheticPeptideFeature=None):
        """Initialize isotopic pattern model.
...
class ModelChargePoissonIsotopicPattern(AbstractModel):
    ...
    """
    def __init__(self,mz_values:np.ndarray,z_start:int=2,n:int=6,μ_mass:float=None,σ_mass:float=1,σ_iso:float=0.1,λ_hat=None,name:str="",model:pm.Model=None,syn:SyntheticPeptideFeature=None):
