...
class TestIsotopeAveragineDistribution:
    ...
    @pytest.fixture(params=[args1, args2])
    ...
    def test_pdf(self, iso_dist, arguments, loc_at_0):
        ...
        assert (
            y.shape == x.shape
        ), f"Array of densities y shape {y.shape} does not correspond to input array x shape {x.shape}"
    ...
    def test_rvs_to_intensities(self, iso_dist, arguments, rng, loc_at_0):
        loc = self.get_loc(loc_at_0, arguments)
        arguments.pop("x")
        bins_pos, intensities = iso_dist.rvs_to_intensities(
            **arguments, loc=loc, random_state=rng, bin_width=0.0001
        )

    def test_averagine_isotopic(self, iso_dist, arguments):
        mass = arguments["mass"]
        num_peaks = arguments["num_peaks"]
        lambdas = iso_dist.averagine_isotopic(mass, num_peaks)
