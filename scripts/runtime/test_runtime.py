    ...
    for batch_num, batch in enumerate(feature_batchs):
        try:
            aligned_features = AlignedFeatureData(
                data_handle, ids=batch, is_parallel=True, num_data_points=20
            )
        except ValueError:
        ...
        time_start_gpu = time()
        trace_gpu = sample_numpyro_nuts(model=model, chain_method="vectorized")
...
).melt(id_vars="batch_number", var_name="Label", value_name="Time")

Fig, ax = plt.subplots()
Fig.set_dpi(300)
sns.pointplot(
    data=plot_df, x="batch_number", y="Time", ax=ax, hue="Label", markers=["^", "o"]
)
