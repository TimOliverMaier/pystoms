...
for module in important_modules:
    ...
    meta_data[module + "_current_commit"] = subprocess.run(
        ["git", "rev-parse", "HEAD"],
        text=True,
        cwd=sys.modules[module].__path__[0],
        check=True,
        capture_output=True,
    ).stdout
...
if __name__ == "__main__":
    ...
    if random_features:
        feature_ids = rng.integers(
            low=0, high=precursor_table.Id.max(), size=number_of_features
        )
    else:
        feature_ids = user_selected_feature_ids
    # parallel
    parallel_features = AlignedFeatureData(
        data_handle, ids=feature_ids, num_data_points=num_data_points
    )
    parallel_features.feature_data.to_netcdf(folder_output_parallel + "input_data.nc")
    ...
    for feature in accepted_feature_ids:
        ...
        trace.to_netcdf(
            f"{folder_output_parallel}/inference_data/inference_data_{feature}.nc"
        )
    # not parallel
    for feature in feature_ids:
        try:
            single_feature = AlignedFeatureData(
                data_handle, ids=[feature], is_parallel=False
            )
        except ValueError:
            warning(f"Skipping feature {feature}.")
            continue
        single_feature.feature_data.to_netcdf(
            folder_output_single + f"input_data_{feature}.nc"
        )
