...
for d1,d2,obs_data in zip(D1,D2,dataShapes):
    ...
    with pm.Model() as model:
        ...
        normals = [pm.Normal(f"obs_{i}",mu = mus[i],sigma=1,observed=data.get_value()[i]) for i in range(d2)]
    tcce = time()
    ...
    with model as model:
        ...
        for i in range(d1):
            data_i = obs_data[i]
            pm.set_data({"data": data_i})
            traces_GPUv.append(pm.sampling_jax.sample_numpyro_nuts(chain_method="vectorized",random_seed=2022))
        tgvse = time()
